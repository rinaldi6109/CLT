...
function prepare() {
	...
	function prepareTarget() {
		...
		;
		target.append("g");
		target.append("g").classed("shot",true);
...
function startTimer(time) {
	...
	t = d3.timer(function(elapsed) {
		...
		var elapsed2=ct*(FADEIN+ACTIVE+FADEOUT+FDELAY)
		+between*delay.filter((d,i) => (i<ct)).reduce((total, d) => total + d,0)+d3.now()-tm;
		if (ct<shots) {
	...
	if (target.selectAll("circle.shot").size()>1) {
		...
		function createZebra() {
			...
			cn.forEach(function(dd){
				...
				var area = areaGenerator(pointsArea.slice(1,5));
				var grid= areaGenerator(pointsArea.slice(0,2));
				
				zebra.append("g").classed("rotate0",true)
				
				zebra.select("g.rotate0")
				.append('path')
				.attr('d', grid)
				.attr("fill","#0070b0")
				;
				
				zebra.append("g").classed("rotate90",true)
				.attr("transform","rotate(90)")
				.attr("opacity",0)
				
				zebra.select("g.rotate90")
				.append('path')
				.attr('d', grid)
				.attr("fill","#0070b0")
				;
				
				zebra.append("g").classed("fixed",true)
				
				zebra.select("g.fixed")
				.append('path')
				.attr('d', area)
				.attr("fill","#0070b0")
				;
...
function computeSS() {
	...
	var ss=target.selectAll("circle.shot").nodes().reduce((total, circle) => total + Math.pow(circle.cx.baseVal.value,2)+Math.pow(circle.cy.baseVal.value,2),0); 
	var sigma=Math.sqrt(ss/2/target.selectAll("circle.shot").size());
...
function buildNormal(gpn) {
	...
	function rotateTarget90(gpn) {
		...
		.ease(d3.easeQuadInOut)
		.duration(1000)
		.attr("transform",zebra.select("g.rotate90").attr("transform")+" rotate(90)")
